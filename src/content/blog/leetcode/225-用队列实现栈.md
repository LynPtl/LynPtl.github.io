...
> **参考思路：**[labuladong 栈和队列](https://labuladong.online/zh/algo/data-structure/stack-queue/)
...
- **时间复杂度**：`push`、`top`、`empty` 均为 $O(1)$。`pop` 为 $O(N)$，其中 $N$ 是队列中的元素数量，因为需要将前 $N - 1$ 个元素旋转一圈。
