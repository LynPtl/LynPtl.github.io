...
| 选项 | 作用 | 记忆逻辑 (Mnemonic) | 注意事项 |
| :--- | :--- | :--- | :--- |
| **`-f`** | 指定提取字段 (Fields) | **F**ield | 如 `-f 1` (第1列) 或 `-f 1,3` (第1和3列)。 |
...
| 选项 | 作用 | 记忆逻辑 (Mnemonic) | 注意事项 |
| :--- | :--- | :--- | :--- |
| **`-name`** | 按文件名查找 | **Name** | 支持通配符，如 `'*.log'`。 |
| **`-type`** | 按文件类型查找 | **Type** | `f` 代表普通文件，`d` 代表目录。 |
| **`-mtime`** | 按修改时间查找 | **M**odification **Time** | `-1` 表示 24 小时内修改过的。 |

### **10. `xargs`**
*   **功能**：参数构建器。将标准输入转换为命令行参数。
*   **记忆逻辑**：当管道后续命令不支持标准输入（如 `rm` 或 `cp`）时使用。

---

## 最佳实践：管道 (Pipeline) 哲学

在实战中，这些命令的威力源自 **组合**。遵循以下逻辑链条，你可以解决 90% 的文本处理需求：
...
4.  **`sort`**: 对数据进行预处理。
5.  **`uniq -c`**: 进行统计汇总。
6.  **`sort -rn`**: 根据频率结果进行重排序。

**示例**：统计访问日志中 TOP 10 的 IP 地址
